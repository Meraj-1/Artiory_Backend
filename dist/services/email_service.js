...
exports.sendShipmentNotificationEmail = sendShipmentNotificationEmail;
 ...
 */
async function handleSuccessfulPayment(orderId, customerEmail, amount) {
 ...
 */
async function sendOrderConfirmationEmails(orderIdOrDoc) {
    try {
        let order = orderIdOrDoc;
        if (typeof order === "string" || order instanceof mongoose_1.default.Types.ObjectId) {
            order = await Order_model_1.default.findById(order).populate("user");
        }
        if (!order) {
 ...
 */
async function sendShipmentNotificationEmail(orderIdOrDoc) {
    try {
        let order = orderIdOrDoc;
        if (typeof order === "string" || order instanceof mongoose_1.default.Types.ObjectId) {
            order = await Order_model_1.default.findById(order).populate("user");
