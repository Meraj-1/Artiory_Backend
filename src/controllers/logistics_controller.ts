 ...
 */
const postToiThink = (endpointPath: string, payload: any): Promise<any> => {
 ...
 */
export const shipOrderWithiThink = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (totalWeightGrams <= 0) totalWeightGrams = 200; // default 200 gm
 ...
 */
export const trackiThinkShipment = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (Array.isArray(awbInput)) {
      ...
      const parts = awbInput.split(",").map((s: string) => s.trim()).filter(Boolean);
    ...
    if (trackingResponse && (trackingResponse.status_code === 200 || trackingResponse.status === "success") && trackingResponse.data) {
      ...
      await Promise.all(
        awbKeys.map(async (awb) => {
          const itemData = trackingMap[awb];
          if (!itemData) return;

          const currentStatus = itemData.current_status || itemData.status || "";
          const statusInfo = ITHINK_STATUS_CODES[currentStatus] || null;

          try {
            const orderDoc = await Order.findOne({ awbNumber: awb });
            if (orderDoc) {
      ...
      return res.status(200).json({
        ...
        data: trackingMap,
        statusCodeReference: ITHINK_STATUS_CODES
      });
    } else {
      ...
      try {
        const orderDoc = await Order.findOne({
          $or: [
            ...
            ...(mongoose.Types.ObjectId.isValid(awbInput.toString().trim()) ? [{ _id: awbInput.toString().trim() }] : [])
          ]
        }).populate("user");
 ...
 */
export const checkPincodeServiceability = async (req: Request, res: Response): Promise<any> => {
  try {
 ...
 */
export const getShippingCharge = async (req: Request, res: Response): Promise<any> => {
  try {
    const {
      from_pincode = 400071,
      to_pincode,
    ...
    if (Array.isArray(orderItems) && orderItems.length > 0) {
      for (const item of orderItems) {
        ...
        if (!itemWeight && prodId && mongoose.Types.ObjectId.isValid(prodId)) {
          try {
            const pDoc = await Product.findById(prodId);
            if (pDoc) {
              const raw = Number(pDoc.weight || (pDoc as any).weightGrams || 150);
    ...
    if (ITHINK_ACCESS_TOKEN && ITHINK_SECRET_KEY) {
      ...
      if (response && (response.status === "success" || response.status_code === 200) && Array.isArray(response.data)) {
        ...
        const couriers = response.data.filter((c: any) => {
          if (isCod) return c.cod === "Y" || c.cod === "yes" || c.cod === 1;
          return c.prepaid === "Y" || c.prepaid === "yes" || c.prepaid === 1;
        });

        if (couriers.length > 0) {
          // Sort couriers by live rate ascending (cheapest / best rate first)
          couriers.sort((a: any, b: any) => Number(a.rate || 999) - Number(b.rate || 999));
 ...
 */
export const getiThinkOrderDetails = async (req: Request, res: Response): Promise<any> => {
  try {
 ...
 */
export const getiThinkShippingLabel = async (req: Request, res: Response): Promise<any> => {
  try {
    const awbFromParams = req.params?.awbNumber;
    const {
      awb_numbers,
 ...
 */
export const renderOrderLabelHtml = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const orderDoc = await Order.findOne({
      $or: [
        ...(identifier && mongoose.Types.ObjectId.isValid(identifier) ? [{ _id: identifier }] : []),
    ...
    const customerName = orderDoc.shippingAddress?.name || (orderDoc.user as any)?.name || "Valued Customer";
    const customerPhone = orderDoc.shippingAddress?.phone || (orderDoc.user as any)?.number || "N/A";
 ...
 */
export const getiThinkCustomerInvoice = async (req: Request, res: Response): Promise<any> => {
  try {
    const awbFromParams = req.params?.awbNumber;
 ...
 */
export const renderOrderInvoiceHtml = async (req: Request, res: Response): Promise<any> => {
  try {
    const { orderId, awbNumber } = req.params;
    const rawIdentifier = orderId || awbNumber;
    const identifier = Array.isArray(rawIdentifier) ? rawIdentifier[0] : (rawIdentifier || "");
    ...
    const customerName = orderDoc.shippingAddress?.name || (orderDoc.user as any)?.name || "Valued Customer";
    const customerPhone = orderDoc.shippingAddress?.phone || (orderDoc.user as any)?.number || (orderDoc.user as any)?.phone || "N/A";
    const customerEmail = orderDoc.shippingAddress?.email || (orderDoc.user as any)?.email || "customer@artiory.com";
    ...
    <table class="items-table">
      ...
      <tbody>
        ${orderDoc.orderItems.map((item, idx) => {
          const productObj = typeof item.productId === "object" ? (item.productId as any) : null;
          const itemName = item.name || productObj?.productName || "Handcrafted Consumer Product";
          const skuCode = productObj?.skuCode || (item as any)?.sku || (item as any)?.skuCode || `SKU-${shortId}`;
 ...
 */
export const getiThinkStates = async (req: Request, res: Response): Promise<any> => {
  try {
 ...
 */
export const getiThinkCities = async (req: Request, res: Response): Promise<any> => {
  try {
 ...
 */
export const addiThinkWarehouse = async (req: Request, res: Response): Promise<any> => {
  try {
    const {
      company_name,
      address1,
    ...
    if (city_id) payload.data.city_id = Number(city_id);
    if (state_id) payload.data.state_id = Number(state_id);
    if (gps) payload.data.gps = gps.toString().trim();
 ...
 */
export const getiThinkWarehouses = async (req: Request, res: Response): Promise<any> => {
  try {
 ...
 */
export const getiThinkZoneRates = async (req: Request, res: Response): Promise<any> => {
  try {
    const {
      from_pincode = 400001,
      shipping_length_cms,
 ...
 */
export const getiThinkStores = async (req: Request, res: Response): Promise<any> => {
  try {
 ...
 */
export const getiThinkStoreOrderDetails = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (Array.isArray(orderListInput)) {
      ...
      const parts = orderListInput.split(",").map((s: string) => s.trim()).filter(Boolean);
 ...
 */
export const getiThinkStoreOrderList = async (req: Request, res: Response): Promise<any> => {
  try {
 ...
 */
export const handleNDRAction = async (req: Request, res: Response): Promise<any> => {
  try {
    const {
      awb_numbers,
      awbNumber,
      orderId,
      ndr_action = 1, // 1 => Reattempt, 2 => RTO
      reattempt_date,
      reattempt_time = "12:00:00",
      reattempt_mobile_number,
      reattempt_address,
      reattempt_address_type = 1, // 1 => Home, 2 => Office
      rto_remark = "Customer requested cancellation / Undeliverable"
    } = req.body || {};
    ...
    if (response && (response.status_code === 200 || response.status === "success")) {
      ...
      try {
        const orderDoc = await Order.findOne({ awbNumber: awbInput.toString().trim() });
        if (orderDoc) {
          if (actionNum === 2) {
            orderDoc.shipmentStatus = "RTO";
          } else {
